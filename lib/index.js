...
readFile(_path2.default.join(__dirname, 'android/app/src/main/res/values/strings.xml')).then(function () {
  ...
  _commander2.default.version('2.2.2').arguments('<newName>').option('-b, --bundleID [value]', 'Set custom bundle identifier eg. "com.junedomingo.travelapp"').option('-d, --displayName [value]', 'Set custom display name eg. "travelapp"').option('-c, --androidColors [value]', 'Set custom colors for android NavigationBar and StatusBar name eg. "#ffffff#222222" old color then new color in hex').action(function (newName) {
    ...
    //colors
    var androidColorsStr = _commander2.default.androidColors || '#ffffff#ffffff';
    var androidColors = androidColorsStr.split('#');
    var oldNavigationBarColor = androidColors[1] || '';
    var navigationBarColor = androidColors[2] || '';
    var oldStatusBarColor = androidColors[3] || '';
    var statusBarColor = androidColors[4] || '';
