...
readFile(_path2.default.join(__dirname, 'android/app/src/main/res/values/strings.xml')).then(function () {
  // const $ = cheerio.load(data);
  // const currentAppName = $('string[name=app_name]').text();
  var currentAppName = _app2.default.name;
  var currentDisplayName = _app2.default.displayName;
  var nS_CurrentAppName = currentAppName.replace(/\s/g, '');
  var lC_Ns_CurrentAppName = nS_CurrentAppName.toLowerCase();

  _commander2.default.version('2.2.2').arguments('<newName>').option('-b, --bundleID [value]', 'Set custom bundle identifier eg. "com.junedomingo.travelapp"').option('-d, --displayName [value]', 'Set custom display name eg. "travelapp"').option('-c, --colors [value]', 'Set custom colors for android NavigationBar and StatusBar name eg. "#ffffff#222222" old color then new color in hex').action(function (newName) {
    ...
    var newDisplayName = _commander2.default.displayName || newName;

    //colors
    var colorsStr = _commander2.default.colors || '#ffffff#ffffff';
    var colors = colorsStr.split('#');
    var oldNavigationBarColor = colors[1] || '';
    var navigationBarColor = colors[2] || '';
    var oldStatusBarColor = colors[3] || '';
    var statusBarColor = colors[4] || '';

    var newBundlePath = void 0;
    var listOfFoldersAndFiles = (0, _foldersAndFiles.foldersAndFiles)(currentAppName, newName);
    var listOfFilesToModifyContent = (0, _filesToModifyContent.filesToModifyContent)(currentAppName, newName, currentDisplayName, newDisplayName, oldNavigationBarColor, navigationBarColor, oldStatusBarColor, statusBarColor);
    ...
    var resolveFoldersAndFiles = new Promise(function (resolve) {
      listOfFoldersAndFiles.forEach(function (element, index) {
        ...
        var successMsg = '/' + dest + ' ' + colors.green('RENAMED');
        ...
        readFile(_path2.default.join(__dirname, 'android/app/src/main/AndroidManifest.xml')).then(function (data) {
          ...
          try {
            for (var _iterator2 = javaFiles[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
              ...
              itemsProcessed++;
              var successMsg = newBundlePath + ' ' + colors.green('BUNDLE INDENTIFIER CHANGED');
    ...
    var rename = function rename() {
      resolveFoldersAndFiles.then(resolveFilesToModifyContent).then(resolveJavaFiles).then(resolveBundleIdentifiers).then(deletePreviousBundleDirectory).then(cleanBuilds).then(function () {
        ...
        return console.log('' + colors.yellow('Please make sure to run "watchman watch-del-all" and "npm start --reset-cache" before running the app. '));
      });
    };
